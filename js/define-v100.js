...
const fmsSU=8;
const fmsSD=9;
const fmsTL=10;  //Tilt L
const fmsTR=11;  //Tilt R
...
let i=0;
var vkPOS = new Array();
//三维数组[0]是KeyboardEvent.code属性值，[1]是显示在软件界面上的键名，[2]是该键的HID码
vkPOS[i++]=["KeyA", "A", 0x04];
vkPOS[i++]=["KeyB", "B", 0x05];
vkPOS[i++]=["KeyC", "C", 0x06];
vkPOS[i++]=["KeyD", "D", 0x07];
...
vkPOS[i++]=["F13", "F13", 0x68];  //Chrome48里是F17
...
vkPOS[i++]=["ContextMenu", "App", 0x65]; //App
vkPOS[i++]=["ControlLeft", "Ctrl", 0xE0]; //Ctrl L
vkPOS[i++]=["ShiftLeft", "Shift", 0xE1];
vkPOS[i++]=["AltLeft", "Alt", 0xE2];
vkPOS[i++]=["MetaLeft", "Win", 0xE3];  //Win L
vkPOS[i++]=["ControlRight", "RCtrl", 0xE4];  //Ctrl R
vkPOS[i++]=["ShiftRight", "RShift", 0xE5];
vkPOS[i++]=["AltRight", "RAlt", 0xE6];
vkPOS[i++]=["MetaRight", "RWin", 0xE7];  //Win R
...
vkPOS[i++]=["NumpadDivide", "Num/", 0x54];  //Num /
vkPOS[i++]=["NumpadMultiply", "Num*", 0x55];  //Num *
vkPOS[i++]=["NumpadSubtract", "Num-", 0x56];  //Num -
vkPOS[i++]=["NumpadAdd", "Num+", 0x57];  //Num +
vkPOS[i++]=["NumpadDecimal", "Num.", 0x63]; //Num .
...
vkPOS[i++]=["IntlBackslash", "K45", 0x64];  //K45
vkPOS[i++]=["LButton", "LButton", 0xf0];   //鼠标键，通过分析原厂app的宏数据得知
vkPOS[i++]=["RButton", "RButton", 0xf1];
vkPOS[i++]=["MButton", "MButton", 0xf2];


function _0bb17qc4rf1e(name){
    for(let i=0; i<vkPOS.length; i++){
        if(name==vkPOS[i][0]) return vkPOS[i][2];
    }
    return 0;
}
function _2h3oo8pkib9g(name){
    for(let i=0; i<vkPOS.length; i++){
        if(name==vkPOS[i][1]) return vkPOS[i][2];
    }
    return 0;
}
function _ffw0a6dpikxi(code){
    for(let i=0; i<vkPOS.length; i++){
        if(code==vkPOS[i][2]) return vkPOS[i][1];
    }
    return "";
}
function _rzc63l9buubi(name){   //参数是KeyboardEvent.code的值，即浏览器传过来的键名
    for(let i=0; i<vkPOS.length; i++){
